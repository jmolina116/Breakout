...
import util.Ball;
import util.Brick;
import util.BrickConfiguration;
import util.BrickTier1;
import util.BrickTier2;
import util.BrickTier3;
import util.Paddle;

import static util.Parameters.BACKGROUND;
import static util.Parameters.BRICK_GAP;
import static util.Parameters.BRICK_TOTAL;
import static util.Parameters.PADDLE_SPEED;
import static util.Parameters.SCORE_HEIGHT;
import static util.Parameters.SCORE_WIDTH;
...
public final class Breakout {
  ...
  private static final class GamePanel extends JPanel {
    ...
    private final ImageIcon background = new ImageIcon(getClass().getResource(BACKGROUND));
    private Ball ball;
    ...
    public void paintComponent(final Graphics g) {
      ...
      if (info) {
        ...
        final Brick scorebar = new Brick(0, 0, SCORE_WIDTH, SCORE_HEIGHT, Color.DARK_GRAY);
        scorebar.paint(g2);
        g2.setColor(Color.YELLOW);
        g2.setFont(new Font("Serif", Font.PLAIN, 20));
        g2.drawString("Bricks Destroyed: " + score, BRICK_GAP, 20);
        g2.drawString("Bricks Remaining: " + (BRICK_TOTAL - score), 300, 20);
      }
    }

    private final class GameMotion implements ActionListener {

      public void actionPerformed(final ActionEvent evt) {
        if (!dead && score != BRICK_TOTAL) {
          ball.move();
          paddle.move();
          checkForHit();
          repaint();
    ...
    private final class KeyMover implements KeyListener {
      ...
      public void keyPressed(final KeyEvent evt) {
        ...
        if (key == KeyEvent.VK_LEFT) {
          paddle.setSpeed(-1 * PADDLE_SPEED);
        }
        if (key == KeyEvent.VK_RIGHT) {
          paddle.setSpeed(PADDLE_SPEED);
        }

        // restart game using SPACE bar
